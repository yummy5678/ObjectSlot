...
int main()
{
    ...
    // ==================================================
    PrintCategory("実使用パターンの速度比較");
    // ==================================================

    // ベンチマーク用の定数
    constexpr int SCENE_OBJECT_COUNT = 10000;   // シーン内のオブジェクト数
    constexpr int FRAME_COUNT = 1000;           // フレーム数
    constexpr int SPAWN_DESTROY_PER_FRAME = 50; // フレームあたりの生成・破棄数

    std::cout << "\n  計測条件:" << std::endl;
    std::cout << "    シーン内オブジェクト数     : " << SCENE_OBJECT_COUNT << std::endl;
    std::cout << "    フレーム数                 : " << FRAME_COUNT << std::endl;
    std::cout << "    フレームあたりの生成・破棄 : " << SPAWN_DESTROY_PER_FRAME << std::endl;

    // ========================================================
    // シナリオ1: シーンロード（一括作成 + ポインタ保持）
    // ========================================================
    std::cout << "\n  --- シナリオ1: シーンロード ---" << std::endl;
    {
        // ObjectSlot版
        auto& pool = ObjectSlotSystem<BenchData>::GetInstance();
        pool.Clear();
        pool.Reserve(SCENE_OBJECT_COUNT);

        auto slotStart = std::chrono::high_resolution_clock::now();
        std::vector<SlotPtr<BenchData>> slotObjects;
        slotObjects.reserve(SCENE_OBJECT_COUNT);
        for (int i = 0; i < SCENE_OBJECT_COUNT; ++i) {
            slotObjects.push_back(pool.Create(BenchData{
                static_cast<float>(i), 0.0f, 0.0f, i }));
        }
        auto slotEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds slotNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            slotEnd - slotStart).count() / SCENE_OBJECT_COUNT;

        // shared_ptr版
        auto sharedStart = std::chrono::high_resolution_clock::now();
        std::vector<std::shared_ptr<BenchData>> sharedObjects;
        sharedObjects.reserve(SCENE_OBJECT_COUNT);
        for (int i = 0; i < SCENE_OBJECT_COUNT; ++i) {
            sharedObjects.push_back(std::make_shared<BenchData>(BenchData{
                static_cast<float>(i), 0.0f, 0.0f, i }));
        }
        auto sharedEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds sharedNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            sharedEnd - sharedStart).count() / SCENE_OBJECT_COUNT;

        PrintBenchmark("シーンロード（1オブジェクトあたり）", slotNs, sharedNs);
    }

    // ========================================================
    // シナリオ2: 毎フレームの一括データ更新（位置計算を想定）
    // ========================================================
    std::cout << std::endl;
    {
        // ObjectSlot版：作成
        ...
        // ObjectSlot版：毎フレーム全オブジェクトの位置を更新
        auto slotStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            for (auto& ptr : slotObjects) {
                BenchData* data = ptr.Get();
                data->x += data->y * deltaTime;
                data->y += 9.8f * deltaTime;
                data->z += data->x * deltaTime;
                sink = data->z;
            }
        }
        auto slotEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds slotNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            slotEnd - slotStart).count() / (static_cast<long long>(FRAME_COUNT) * SCENE_OBJECT_COUNT);

        // shared_ptr版：同じ処理
        auto sharedStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            for (auto& ptr : sharedObjects) {
                BenchData* data = ptr.get();
                data->x += data->y * deltaTime;
                data->y += 9.8f * deltaTime;
                data->z += data->x * deltaTime;
        ...
        auto sharedEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds sharedNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            sharedEnd - sharedStart).count() / (static_cast<long long>(FRAME_COUNT) * SCENE_OBJECT_COUNT);

        PrintBenchmark("毎フレーム位置更新（1オブジェクト1フレームあたり）", slotNs, sharedNs);
    }

    // ========================================================
    // シナリオ3: ゲーム中の生成・破棄（弾やエフェクトを想定）
    // ========================================================
    std::cout << std::endl;
    {
        // ObjectSlot版
        auto& pool = ObjectSlotSystem<BenchData>::GetInstance();
        pool.Clear();
        pool.Reserve(SCENE_OBJECT_COUNT + SPAWN_DESTROY_PER_FRAME);
        ...
        auto slotStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            // 末尾からSPAWN_DESTROY_PER_FRAME個を破棄
            for (int j = 0; j < SPAWN_DESTROY_PER_FRAME; ++j) {
                slotObjects.back().Reset();
                slotObjects.pop_back();
            }
            // 同数を新規作成
            for (int j = 0; j < SPAWN_DESTROY_PER_FRAME; ++j) {
                slotObjects.push_back(pool.Create(BenchData{
                    static_cast<float>(j), 0.0f, 0.0f, j }));
            }
        }
        auto slotEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds slotNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            slotEnd - slotStart).count() / (static_cast<long long>(FRAME_COUNT) * SPAWN_DESTROY_PER_FRAME);
        ...
        auto sharedEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds sharedNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            sharedEnd - sharedStart).count() / (static_cast<long long>(FRAME_COUNT) * SPAWN_DESTROY_PER_FRAME);

        PrintBenchmark("ゲーム中の生成・破棄（1オブジェクトあたり）", slotNs, sharedNs);
    }

    // ========================================================
    // シナリオ4: ポリモーフィック描画ループ（基底型でイテレーション）
    // ========================================================
    std::cout << std::endl;
    {
        // ObjectSlot版：作成
        ...
        std::vector<SlotRef<IBenchObject>> slotRefs;
        std::vector<SignalSlotPtr<BenchObject>> slotOwners;
        slotRefs.reserve(SCENE_OBJECT_COUNT);
        slotOwners.reserve(SCENE_OBJECT_COUNT);
        ...
        // shared_ptr版：作成
        std::vector<std::shared_ptr<IBenchObject>> sharedPtrs;
        sharedPtrs.reserve(SCENE_OBJECT_COUNT);
        for (int i = 0; i < SCENE_OBJECT_COUNT; ++i) {
            sharedPtrs.push_back(std::make_shared<BenchObject>(static_cast<float>(i)));
        }
        ...
        // ObjectSlot版：FRAMEフレーム分の描画ループ
        auto slotStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            for (auto& ref : slotRefs) {
                sink = ref->GetValue();
            }
        }
        auto slotEnd = std::chrono::high_resolution_clock::now();
        ...
        PrintBenchmark("描画ループ（基底型で1オブジェクト1フレームあたり）", slotNs, sharedNs);
    }

    // ========================================================
    // シナリオ5: 所有権の共有（複数箇所からの参照保持）
    // ========================================================
    std::cout << std::endl;
    {
        // ObjectSlot版
        ...
        // オーナー配列
        std::vector<SlotPtr<BenchData>> owners;
        owners.reserve(SCENE_OBJECT_COUNT);
        for (int i = 0; i < SCENE_OBJECT_COUNT; ++i) {
            owners.push_back(pool.Create(BenchData{
                static_cast<float>(i), 0.0f, 0.0f, i }));
        }

        // shared_ptr版
        std::vector<std::shared_ptr<BenchData>> sharedOwners;
        sharedOwners.reserve(SCENE_OBJECT_COUNT);
        for (int i = 0; i < SCENE_OBJECT_COUNT; ++i) {
            sharedOwners.push_back(std::make_shared<BenchData>(BenchData{
                static_cast<float>(i), 0.0f, 0.0f, i }));
        }

        volatile float sink = 0.0f;

        // ObjectSlot版：コピーして参照を増やし、アクセスして、破棄
        auto slotStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            // サブシステムが参照のコピーを受け取る
            std::vector<SlotPtr<BenchData>> subSystemRefs(
                owners.begin(), owners.end());
            // サブシステムが全要素にアクセス
            for (auto& ptr : subSystemRefs) {
                sink = ptr->x;
            }
            // サブシステムの参照が破棄される（スコープ終了）
        }
        auto slotEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds slotNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            slotEnd - slotStart).count() / (static_cast<long long>(FRAME_COUNT) * SCENE_OBJECT_COUNT);
        ...
        auto sharedStart = std::chrono::high_resolution_clock::now();
        for (int frame = 0; frame < FRAME_COUNT; ++frame) {
            std::vector<std::shared_ptr<BenchData>> subSystemRefs(
                sharedOwners.begin(), sharedOwners.end());
            for (auto& ptr : subSystemRefs) {
                sink = ptr->x;
            }
        }
        ...
        PrintBenchmark("所有権共有（コピー+アクセス+破棄を1フレームあたり）", slotNs, sharedNs);
    }

    // ========================================================
    // シナリオ6: リソース解放通知（デバイス破棄→バッファ自動解放）
    // ========================================================
    std::cout << std::endl;
    {
        constexpr int DEVICE_BUFFER_COUNT = 10000;

        // ObjectSlot版
        auto& devicePool = SignalSlotSystem<BenchData>::GetInstance();
        auto& bufferPool = ObjectSlotSystem<BenchData>::GetInstance();
        devicePool.Clear();
        bufferPool.Clear();
        bufferPool.Reserve(DEVICE_BUFFER_COUNT);

        auto device = devicePool.Create(BenchData{ 0.0f, 0.0f, 0.0f, 0 });

        std::vector<SlotPtr<BenchData>> buffers;
        std::vector<Subscription<BenchData>> subs;
        buffers.reserve(DEVICE_BUFFER_COUNT);
        subs.reserve(DEVICE_BUFFER_COUNT);

        for (int i = 0; i < DEVICE_BUFFER_COUNT; ++i) {
            auto buf = bufferPool.Create(BenchData{
                static_cast<float>(i), 0.0f, 0.0f, i });
            buffers.push_back(buf);
            // デバイス解放時にバッファを自動解放する購読
            subs.push_back(device.Subscribe([buf]() mutable {
                buf.Reset();
                }));
        }

        // デバイス解放→全購読者にコールバック→全バッファ解放
        auto slotStart = std::chrono::high_resolution_clock::now();
        device.Reset();
        auto slotEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds slotNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            slotEnd - slotStart).count() / DEVICE_BUFFER_COUNT;

        // shared_ptr版：shared_ptrにはこの機能がないため、
        // weak_ptr + 手動チェックで疑似的に再現
        auto sharedDevice = std::make_shared<BenchData>(BenchData{ 0.0f, 0.0f, 0.0f, 0 });

        struct BufferEntry {
            std::shared_ptr<BenchData> buffer;
            std::weak_ptr<BenchData> deviceRef;
        };
        std::vector<BufferEntry> sharedBuffers;
        sharedBuffers.reserve(DEVICE_BUFFER_COUNT);

        for (int i = 0; i < DEVICE_BUFFER_COUNT; ++i) {
            sharedBuffers.push_back({
                std::make_shared<BenchData>(BenchData{
                    static_cast<float>(i), 0.0f, 0.0f, i }),
                sharedDevice
                });
        }

        // デバイス解放→手動で全バッファをチェック・解放
        auto sharedStart = std::chrono::high_resolution_clock::now();
        sharedDevice.reset();
        for (auto& entry : sharedBuffers) {
            if (entry.deviceRef.expired()) {
                entry.buffer.reset();
            }
        }
        auto sharedEnd = std::chrono::high_resolution_clock::now();
        Nanoseconds sharedNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
            sharedEnd - sharedStart).count() / DEVICE_BUFFER_COUNT;

        PrintBenchmark("リソース解放通知（1バッファあたり）", slotNs, sharedNs);
    }
