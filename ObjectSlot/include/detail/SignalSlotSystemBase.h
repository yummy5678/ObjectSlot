 ...
 * 要素が解放される際に、登録された全てのコールバックを実行する。
 *
 * 主な特徴:
 * - ObjectSlotSystemBaseの全機能（連続配置、参照カウント、世代番号）
 * - 要素ごとの購読リスト管理
 * - Subscriptionオブジェクトによる購読の自動解除
 *
 * @tparam T 管理する要素の型
 */
template<typename T>
...
public:
     ...
     * @brief プール内の全要素を削除
     *
     * 全ての有効な要素に対して購読者への通知を行い、
     * プールを初期状態にリセットする。
     *
     * @warning 残っているSystemは無効になる
     */
    void Clear() {
...
protected:
    ...
    void RemoveSubscription(uint32_t slotIndex, uint32_t subscriptionId) {
        if (slotIndex >= m_subscriptions.size()) {
            return;
        }
    ...
    /**
     * @brief 指定スロットの全購読者に通知
     *
     * 登録されている全てのコールバックを実行する。
